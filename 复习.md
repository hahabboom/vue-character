...
Promise很好的解决了毁掉地狱的went8i，但是也存在一些缺点，比如无法取消promise，错误需要通过回调函数捕获
...
2. 协商缓存（如果缓存过期，就需要发起请求验证资源是否有更新；Last-Modified和ETag）
   ...
   但是Last-Modified只能以秒计时，如果在补课感知的时间内修改完文件，那么服务端会认为资源还是命中了，不会返回正确资源
...
跨站请求伪造，攻击者诱导受害者进入第三方网站，在第三方网站，向被攻击网站发起跨域请求。利用受害者在被攻击网站已获取的注册凭证，绕过后台的用户验证，达到冒充用户对呗攻击的王振执行某项操作的目的。CSRF利用网站对用户浏览器的信任。
